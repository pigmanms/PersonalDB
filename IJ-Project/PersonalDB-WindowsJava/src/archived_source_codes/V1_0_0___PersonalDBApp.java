...
/**
 * archived_source_codes.V1_0_0___PersonalDBApp — a Windows-friendly Java Swing/AWT desktop app for managing a personal database (SPPD).
 *
 ...
 *
 * No external libraries required. Compile with: javac archived_source_codes.V1_0_0___PersonalDBApp.java
 * Run with: java archived_source_codes.V1_0_0___PersonalDBApp
 */
public class V1_0_0___PersonalDBApp extends JFrame {
    // ---- Model ----
    ...
    private JMenuBar buildMenuBar() {
        ...
        miAbout.addActionListener(e -> JOptionPane.showMessageDialog(this,
                "archived_source_codes.V1_0_0___PersonalDBApp — Java Swing\n" +
    ...
    static class PersonComboRenderer extends DefaultListCellRenderer {
        @Override public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            JLabel lbl = (JLabel) super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
            if (value instanceof PersonRecord) lbl.setText(((PersonRecord) value).displayName(null));
            return lbl;
        }
    }
    ...
    // --- Schema table model ---
    static class SchemaTableModel extends AbstractTableModel {
        private final Schema schema;
        private final String[] cols = {"Name", "Type", "Enum Options"};
        public SchemaTableModel(Schema schema) { this.schema = schema; }
        public int getRowCount() { return schema.fields.size(); }
        public int getColumnCount() { return cols.length; }
        public String getColumnName(int c){ return cols[c]; }
        public Object getValueAt(int r, int c) {
            FieldDefinition f = schema.fields.get(r);
            switch (c) {
        ...
        public boolean isCellEditable(int r, int c) { return c==0 || c==2; }
        public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
            FieldDefinition f = schema.fields.get(rowIndex);
            if (columnIndex==0) f.name = String.valueOf(aValue);
            if (columnIndex==2) f.enumOptions = Arrays.stream(String.valueOf(aValue).split(",")).map(String::trim).filter(s->!s.isBlank()).collect(Collectors.toList());
        }
    }
    ...
    private void openProject() {
        JFileChooser fc = new JFileChooser();
        fc.setFileFilter(new FileNameExtensionFilter("Personal DB (*.pdb)", "pdb"));
        if (fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            File f = fc.getSelectedFile();
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(f))) {
                PersonalDatabase loaded = (PersonalDatabase) ois.readObject();
                db.schema = loaded.schema;
                db.people = loaded.people;
    ...
    private static File ensureExt(File f, String ext) {
        ...
        if (!n.toLowerCase().endsWith(ext)) {
            return new File(f.getParentFile(), n + ext);
        }
        return f;
    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            new V1_0_0___PersonalDBApp().setVisible(true);
        });
    }
}
